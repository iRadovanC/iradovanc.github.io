...
test('Good contextual answer covers a complete stealth restart and returns to the tablet',()=>{const g=fresh();g.crash();advance(g,3.4);g.act('answer',g.room.question.answers.findIndex(a=>a.good));g.act('restart');advance(g,4.3);assert.equal(g.phase,'tablet');assert.equal(g.rescues,1);assert.equal(g.bugs,0);assert.equal(g.stability>70,true);assert.equal(g.tracked,null);assert.equal(g.room,null);});
...
test('Pause freezes blue death and recovery, and stops hidden restarting',()=>{const g=fresh();g.crash();g.pause();advance(g,12);assert.equal(g.phase,'bsod');assert.equal(g.bsodRemaining,3.3);g.resume();advance(g,3.4);g.act('restart');g.pause();const s=[g.elapsed,g.room.remaining,g.room.progress,g.trust];advance(g,20);g.act('answer',0);assert.deepEqual([g.elapsed,g.room.remaining,g.room.progress,g.trust],s);assert.equal(g.room.restarting,false);});
...
test('Time expiring during recovery cannot award a win until the app is restored',()=>{const g=fresh();g.completed=4;g.pluginDemos=1;g.elapsed=89;g.crash();advance(g,3.4);assert.equal(g.remaining,0);assert.equal(g.phase,'room');assert.equal(g.status,'playing');g.act('answer',g.room.question.answers.findIndex(a=>a.good));g.act('restart');advance(g,4.3);assert.equal(g.status,'won');});
