(function(){
  ...
  function renderPlugin(force){
    ...
    if(id==='layers'){const list=document.createElement('div');list.className='plugin-list';list.innerHTML=bad?'<span>✓ Rozzuření delfíni (128)</span><span>✓ Smutek projektového manažera</span><span>☐ Jednotky (stydí se)</span>':'<span>✓ Základní mapa</span><span>☐ Jednotky v reálném čase</span><span>☐ Situace v terénu</span>';content.append(list);}
  ...
  function updateMapEffects(){const el=$('plugin-map-effect');el.classList.toggle('office-map',!!game.pluginBugs.maps);el.classList.toggle('dolphin-map',!!game.pluginBugs.layers);el.innerHTML=game.pluginBugs.maps?'<span class="office-label">KUCHYŇKA</span><span class="office-desk">☕<small>Nabíjecí stanice pro PM</small></span><span class="office-wc">WC / krizové porady</span>':game.pluginBugs.layers?'<span class="dolphin d1">🐬</span><span class="dolphin d2">🐬</span><span class="dolphin d3">🐬</span><span class="dolphin d4">🐬</span><b>VRSTVA: DELFÍNI V REÁLNÉM ČASE</b>':'';}
